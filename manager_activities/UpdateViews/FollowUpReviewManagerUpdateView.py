...
import traceback
class FollowUpReviewManagerUpdateView(UpdateView):
	model=Review
	...
	def get_context_data(self, **kwargs):
		context=super(FollowUpReviewManagerUpdateView,self).get_context_data(**kwargs)
	...
	def form_valid(self, form):
		...
		try:
			...
			handle_exception()
			return super(FollowUpReviewManagerUpdateView,self).form_invalid(form)
		EmailHelper.send_email(request=self.request,
								user=self.request.user,
								review=review_instance,
								is_updated=False,
								follow_up=True)
		return redirect('manager_activities:peer_review_manager_list')
	...
	def dispatch(self, *args, **kwargs):
		return super(FollowUpReviewManagerUpdateView, self).dispatch(*args, **kwargs)
